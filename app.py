...
with tab1:
    ...
    if not api_key_set:
        ...
        if st.session_state.is_scouting:
            with st.status("Scouting the web — this may take a few minutes...", expanded=True) as status:
                ...
                st.write(f"📡 Found **{len(urls)} unique URLs**. Extracting scholarships...")
                run_scholarship_pipeline(urls)
                st.session_state.is_scouting = False
                ...
                if os.path.exists(DB_PATH):
                    ...
                    if len(df) == 0:
                        status.update(
                            label="⚠️ Sites scraped but no scholarships extracted. Try different search terms.",
                            state="error", expanded=False,
                        )
                    else:
                        status.update(
                            label=f"✅ Done! {len(df)} scholarships found · {len(matches)} match your profile.",
                            state="complete", expanded=False,
                        )
                else:
                    status.update(label="⚠️ Pipeline ran but no data was saved.", state="error", expanded=False)
    ...
    if st.session_state.scholarships_df is not None:
        ...
        if st.session_state.scout_done:
            n_matches = len(st.session_state.matches_df) if st.session_state.matches_df is not None else 0
            st.success(
                f"**Step 1 complete!** Found {len(df)} active scholarships — "
                f"{n_matches} match your profile. "
                f"👉 Click the **Matches** tab above to review and filter them."
            )
